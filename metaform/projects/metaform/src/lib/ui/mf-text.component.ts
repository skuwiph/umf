        ...
        <ng-template #edit>
            <ng-container *ngIf="textType == 'multi'; else single">
                <textarea
                    ...
                    class="mfc mf-control-item-multi"
                    name="{{ name }}"
...
export class MetaFormTextComponent extends MetaFormControlBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.control) {
            ...
            if (this.maxLength > 0) {
                this.pattern += `{1,${this.maxLength}}$`;
            }

            this.formControl.setValue(this.form.getValue(this.name));

            this.formControl.valueChanges.pipe(debounceTime(250), distinctUntilChanged()).subscribe(obs => {
